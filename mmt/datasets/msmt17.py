...
class Dataset_MSMT(object):
    def __init__(self, root):
        ...
        self.query_pids, self.gallery_pids = None, None
    ...
    def load(self, verbose=True):
        ...
        self.query, self.query_pids = _pluck_msmt(osp.join(exdir, 'list_query.txt'), 'test')
        self.gallery, self.gallery_pids = _pluck_msmt(osp.join(exdir, 'list_gallery.txt'), 'test')
        ...
        if verbose:
            ...
            print("  query    | {:5d} | {:8d}"
                  .format(len(self.query_pids), len(self.query)))
            print("  gallery  | {:5d} | {:8d}"
                  .format(len(self.gallery_pids), len(self.gallery)))

class MSMT17(Dataset_MSMT):

    def __init__(self, root, verbose=True, split_id=0, download=True):
