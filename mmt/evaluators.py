...
def extract_features(model, data_loader, print_freq=10, metric=None, domain_label=-1, epochs=-1):
    model.eval()
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, _) in enumerate(data_loader):
            ...
            outputs = extract_cnn_feature(model, imgs, domain_label=domain_label, epochs=epochs)
...
class Evaluator(object):
    ...
    def evaluate(self, data_loader, query, gallery, metric=None, cmc_flag=False, rerank=False, pre_features=None,
        ...
        if (pre_features is None):
            features, _ = extract_features(self.model, data_loader, domain_label=domain_label)
        else:
